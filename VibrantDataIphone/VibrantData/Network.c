...
{
  return recvfrom(self->file_descriptor, data, num_bytes, 0, 
...
BOOL  net_tcp_receive_connections (Network* self, int port)
{
  ...
  listen(self->local_file_descriptor, max_number_of_clients);
  if(error < 0)
    {
...
{
  return inet_ntoa(self->remote_address.sin_addr);
}
/*---------------------------------------------------------------------*/
